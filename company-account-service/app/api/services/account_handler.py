...
class AccountHandler:
    ...
    async def __init__(self, account_id : int, company_id : int):
        self._sql_account = await db_manager.SQLAccountManger.get_SQLAccount(account_id, company_id)

    @staticmethod
    async def create_account(new_account : Account, company_id : int) -> Account:
        sqlaccount = await db_manager.SQLAccountManger.get_SQLAccount_with_email(new_account.email)
    ...
    async def delete(self):
        await db_manager.SQLAccountManger.delete_SQLAccount(self._sql_account)
