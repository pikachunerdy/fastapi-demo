...
async def get_token(form_data: OAuth2PasswordRequestForm = Depends()) -> Token:
    ...
    '''
    sqlaccount  = db_manager.SQLAccountManger.get_SQLAccount_with_email(form_data.email)
    if not sqlaccount:
        raise HTTPException(
            status_code=401,
    ...
    token_data.company_id = sqlaccount.company.id
    token_data.account_id = sqlaccount.id
