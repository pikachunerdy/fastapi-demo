...
async def get_account(account_id : str, token_data : TokenData = Depends(token_authentication)) -> AccountInfo:
    if token_data.permission.manage_accounts != True : raise InvalidPermissionException(token_data,"")
    account_handler = await AccountHandler.create(int(account_id), token_data.company_id)
...
async def put_account(account : ModifyAccount = Body(...), token_data : TokenData = Depends(token_authentication)) -> AccountInfo:
    ...
    account_handler = await AccountHandler.create(int(account.id), token_data.company_id)
