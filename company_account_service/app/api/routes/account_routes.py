...
async def post_account(new_account : RegisterAccount = Body(...), token_data : TokenData = Depends(token_authentication)) -> AccountInfo:
    if not token_data.permission.manage_accounts:
        raise InvalidPermissionException(token_data,"")
...
async def put_account(account : ModifyAccount = Body(...), token_data : TokenData = Depends(token_authentication)) -> AccountInfo:
    if not token_data.permission.manage_accounts:
        raise InvalidPermissionException(token_data,"Not able to modify accounts")
...
#     email_link = ('http:/localhost:8001/' if environmentSettings.ENV == 'DEV' else 'tbd') + '/password/'
#     token = PasswordRequestToken(
#         email = email,
#         expiry = time.time() + 60*60*24
#     )
#     jwt.encode(token.dict(), environmentSettings.jwt_secret, algorithm=environmentSettings.jwt_algorithm)
...
#     payload = PasswordRequestToken(**(jwt.decode(token, environmentSettings.jwt_secret, algorithms=[environmentSettings.jwt_algorithm])))
...
#     await handler.change_password(payload.password)
